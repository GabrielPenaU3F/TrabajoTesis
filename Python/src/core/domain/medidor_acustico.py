...
class MedidorAcustico:
    ...
    def medir(self, metodo):
        ...
        thread_medicion = Thread(target=self.efectuar_medicion, args=(metodo,), daemon=True)
    ...
    def efectuar_medicion(self, metodo):
        ...
        try:
            ...
            mensaje = Mensaje(destinatario="VistaPrincipal", mensaje="DispositivoInaccesible")
            self.queue.put(mensaje)
    ...
    def calcular_parametros_acusticos(self, respuesta_impulsional, fs):
        thread_calculo = Thread(target=self.efectuar_calculo, args=(respuesta_impulsional, fs), daemon=True)
